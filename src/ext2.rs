...
impl<'a> Ext2Fs<'a> {
	fn new(disk: &mut IDEDisk) -> Result<Self, String> {
		let superblock = read_block(&mut disk, 1)?;
		...
		if superblock.state != FS_CLEAN {
			return Err(format!("ext2 is not clean state={}", superblock.state))
		}
		...
		let blkgroups_count = superblock.blocks_count / superblock.blocks_per_group + if superblock.blocks_count % superblock.blocks_per_group != 0 { 1 } else { 0 };
		let blkgroups_blk_count = blkgroups_count * mem::size_of::<BlockGroupDescriptor>() / unsafe { ext2_block_size } + if blkgroups_count * mem::size_of::<BlockGroupDescriptor>() % unsafe { ext2_block_size } != 0 { 1 } else { 0 };
		
		let blkgroups = unsafe {*(read_blocks(disk, blkgroups_block, blkgroups_blk_count)?.as_ptr() as *mut Vec<BlockGroupDescriptor>)};
		Ok(Ext2Fs { superblock, disk, blkgroups })
	}

	fn mount(self, mnt_point: &String) {
		unsafe {
			MNT_POINTS.push(MntPoint { path: mnt_point, fs: self });
		}
	}
	...
	fn read_inode(&mut self, ino: u32) -> Result<Inode, String> {
		...
		let inode_grp = inode_i / superblock.inodes_per_group;
		let grp_off = inode_i % superblock.inodes_per_group;

		let grp_block = grp_off * superblock.inode_size / unsafe {ext2_block_size};
		let grp_block_off = grp_off * superblock.inode_size % unsafe {ext2_block_size};

		let grp: BlockGroupDescriptor = self.blkgroups[inode_grp];

		let block = read_block(self.disk, grp.inode_table_block + grp_block)?;
